...
async def main():
    if len(sys.argv) < 2:
        print("Usage: python scripts/get_product_detail_v1.py <mall_product_no>")
        sys.exit(1)
    ...
    result = await client.get_product_detail(mall_product_no)
    product = result.mall_product
    print(f"상품명: {product.product_name}")
    print(f"판매상태: {product.sale_status_type}")
    print(f"판매가: {product.sale_price}")
    print(f"옵션 수: {len(result.mall_product_option_web_models)}개")
